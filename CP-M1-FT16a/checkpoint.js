...
var isAncestor = function(genealogyTree, ancestor, descendant){
  // Tu código aca:
      ...
      for (let i = 0; i < genealogyTree[ancestor].length; i++){
        if(isAncestor(genealogyTree, genealogyTree[ancestor][i], descendant, i)){
        return true;
      };
      };
      return false;
...
var specialSort = function(array, orderFunction) {
  ...
  while(swap){
    ...
    for(let i =0; i<array.length-1; i++){
      if(orderFunction(array[i], array[i+1]) < 0){
        let aux = array[i];
        array[i] = array[i+1];
        array[i+1] = aux;
        swap = true;
...
function closureDetect(symptoms, min) {
  ...
  return function(persona) {
    pSymptoms = persona.symptoms
    if(pSymptoms.length > min) {
      var checkerSymptoms = symptoms.some(v => pSymptoms.includes(v));
      return checkerSymptoms;
    }
    return false;
  }
}
